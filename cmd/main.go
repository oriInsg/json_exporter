...
func probeHandler(w http.ResponseWriter, r *http.Request, logger *slog.Logger, config config.Config) {
	...
	//Check if there are unrecognised query parameters, if there are, try adding them back onto "target"
	otherKeys := checkQueryParams(r)
	if len(otherKeys) > 0 {
		fmt.Println("Unexpected keys found")
		for key, value := range otherKeys {
			target += fmt.Sprintf("&%s=%v", key, strings.Join(value, ", "))
		}
	}
...
func checkQueryParams(r *http.Request) map[string][]string {
	allowedKeys := map[string]bool{
		"target": true,
		"module": true,
	}

	unexpectedKeys := make(map[string][]string)
	queryParams := r.URL.Query()

	for key, values := range queryParams {
		if !allowedKeys[key] {
			unexpectedKeys[key] = values
		}
	}

	if len(unexpectedKeys) > 0 {
		fmt.Println("Unexpected keys found:")
		for key, values := range unexpectedKeys {
